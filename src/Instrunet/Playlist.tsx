...
import { createEffect, createRenderEffect, createSignal, JSX, on, Show } from "solid-js";
import { baseUrl, Kind } from "../Singletons";
import PlayerComponent from "./Components/PlayerComponent";
import { AiOutlineDelete, AiOutlineSwap } from "solid-icons/ai";
import { CgClose, CgInsertBefore } from "solid-icons/cg";
...
const Playlist = () => {
	...
	const SearchPopOver: () => JSX.Element = () => {
		...
		return <div class="drop-shadow-2xl  lg:w-200 md:w-[75vw] w-[95vw] left-0 right-0 mx-auto h-[100vh] fixed p-5 z-50">
			...
			<div class="grid grid-cols-1 lg:max-h-190 max-h-full lg:grid-cols-2 gap-2">
				<div class="flex flex-col justify-center">
					<img class={"lg:max-w-100 max-w-50 mx-auto mt-2"}
						src={baseUrl + "playlist-tmb?asFile=true&playlistuuid=" + params.playlistuuid} onClick={(e) => {
									...
									if (anInput.files && anInput.files[0]) {
										...
										reader.onloadend = () => {
											fetch(baseUrl + "upload-playlist-thumbnail", {
												...
												if (res.ok) {
													(e.target as HTMLImageElement).src = reader.result! as string
												}
											})
										}
									}
						...
						}}></img>
					{
						...
						}} type="text" class="text-center text-2xl font-bold mt-2" value={playlistInfo()?.title}></input> : <div class="text-center text-2xl font-bold mt-2">
							{
								playlistInfo()?.title
							}
						</div>
							...
							<Show when={playlistSongInfo()} keyed={true}>
								{
									playlistSongInfo()!.map((item, index) => {
										return <tr classList={{ ["bg-base-300"]: playCurrentIndex() === index }}
											...
											}}>{playlistInfo()?.content[index] ? <img alt="" class="lg:max-w-15 max-w-10"
